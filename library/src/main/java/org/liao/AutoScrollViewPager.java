...
public class AutoScrollViewPager extends ViewPager {
    ...
    /**
     * <ul>
     * if stop Scroll When Touch is true
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (stopScrollWhenTouch) {
            if ((action == MotionEvent.ACTION_DOWN) && isAutoScroll) {
                ...
                startAutoScroll();
            } else if (ev.getAction() == MotionEvent.ACTION_MOVE && isStopByTouch) {
                startAutoScroll();
            }
        }
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
            ...
            // /**
            // * current index is first one and slide to right or current index
            // is last one and slide to left.<br/>
            // * if slide border mode is to parent, then
            // requestDisallowInterceptTouchEvent false.<br/>
            // * else scroll to last one when current item is first one, scroll
            // to first one when current item is last
            // * one.
            // */
...
//				startAutoScroll();
                ...
                // add by liaojiahui 处理滑动scrollview是在viewpager区域无效的bug
                ...
                if (action == MotionEvent.ACTION_MOVE) {
                    ...
                    if (disX - disY > 10) { // 左右滑动
                        getParent().requestDisallowInterceptTouchEvent(true);
                    } else if (disY - disX > 10) {
                        getParent().requestDisallowInterceptTouchEvent(false);
                    }
                }
//				startAutoScroll();
